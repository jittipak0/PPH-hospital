 ...
 */
const createUser = ({ username, password, role, acceptedPolicies = 0 }) => {
  const timestamp = new Date().toISOString()
  const id = uuidv4()
  db.prepare(
    'INSERT INTO users (id, username, password, role, acceptedPolicies, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?)'
  ).run(id, username, password, role, acceptedPolicies ? 1 : 0, timestamp, timestamp)
  return findById(id)
}
...
const updateUser = (id, updates) => {
  ...
  db.prepare(
    'UPDATE users SET username = ?, password = ?, role = ?, acceptedPolicies = ?, updatedAt = ? WHERE id = ?'
  ).run(next.username, next.password, next.role, next.acceptedPolicies, timestamp, id)
  return findById(id)
}
...
const listUsers = () => {
  return db.prepare('SELECT id, username, role, acceptedPolicies, createdAt, updatedAt FROM users').all()
}
