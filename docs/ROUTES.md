...
| GET | `/api/security/csrf-token` | ไม่ต้องล็อกอิน | ขอ CSRF token ใหม่ (cookie + ใช้ประกอบ header `X-CSRF-Token`) | ควรเรียกก่อนทุก POST/PUT/DELETE | `{ "data": { "csrf_token": "..." }, "meta": { "request_id": "req-abc" } }` |
...
| POST | `/api/forms/medical-record-request` | ไม่ต้องล็อกอิน (ต้องส่ง CSRF + header) | บันทึกคำขอรับสำเนาประวัติการรักษา | Header `X-CSRF-TOKEN`, `X-Requested-With`; body `{ "full_name": "Jane Doe", "hn": "AB123", "citizen_id": "1234567890123", "phone": "081...", "address": "...", "consent": true }` + ไฟล์ `idcard_file` | `201 + { "data": { "id": 10, "submitted_at": "2025-01-04T05:06:07Z" }, "meta": { "request_id": "req-med" } }` |
| POST | `/api/forms/donation` | ไม่ต้องล็อกอิน (ต้องส่ง CSRF + header) | รับแบบฟอร์มบริจาค | `channel` ต้องอยู่ใน `cash|bank|promptpay`, `amount` > 0 | `201 + { "data": { "id": 3, "reference_code": "4F2C1ABCD0", "submitted_at": "2025-01-04T05:06:07Z" }, "meta": { "request_id": "req-donation" } }` |
| POST | `/api/forms/satisfaction` | ไม่ต้องล็อกอิน (ต้องส่ง CSRF + header) | รับแบบประเมินความพึงพอใจ | `score_overall|score_waittime|score_staff` อยู่ระหว่าง 1-5 | `201 + { "data": { "id": 5, "submitted_at": "2025-01-04T05:06:07Z" }, "meta": { "request_id": "req-survey" } }` |
| POST | `/api/programs/health-rider/apply` | ไม่ต้องล็อกอิน (ต้องส่ง CSRF + header) | สมัครโครงการ Health Rider | ระบุ `full_name`, `address`, `district`, `province`, `zipcode`, `phone`, `consent` | `201 + { "data": { "id": 7, "submitted_at": "2025-01-04T05:06:07Z" }, "meta": { "request_id": "req-rider" } }` |
